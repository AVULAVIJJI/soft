...
export default function ForgotPassword() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch(`${API}/api/v1/auth/forgot-password`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({email, portal:"admin"}) });
      const d = await r.json();
      if (r.ok) { setSent(true); if(d.reset_url) setError(d.reset_url); } else { setError(d.detail||"Failed"); }
  ...
  return (
    <div style={S.wrap}><div style={S.card}>
      ...
      <div style={S.sub as React.CSSProperties}>ADMIN PORTAL</div>
      {sent ? (
        <div style={S.success as React.CSSProperties}>
          ...
          <p style={{color:"#6b7fa3",fontSize:"14px",lineHeight:1.7,marginBottom:"24px"}}>
            {error && error.startsWith("http") ? (
              <><span style={{color:"#4ade80"}}>Dev Mode - Click link below:</span><br/><a href={error} style={{color:"#60a5fa",wordBreak:"break-all"}}>{error}</a></>
            ) : "A password reset link has been sent if this email is registered. Check inbox and spam."}
          </p>
